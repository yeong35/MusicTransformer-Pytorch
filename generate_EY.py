...
def main():
    ...
    classic_path = './dataset/JH_EY_classic/'
    pop_path = './dataset/JH_EY_pop/'

    _, _, classic_dataset = create_epiano_datasets(classic_path, args.num_prime, random_seq=False)
    _, _, pop_dataset = create_epiano_datasets(pop_path, args.num_prime, random_seq=False)

    classic_index_list = list(range(len(classic_dataset)))
    pop_index_list = list(range(len(pop_dataset)))
    ...
    for classic_index in classic_index_list:
        primer, _, _ = classic_dataset[classic_index]
        primer = primer.to(get_device())
        print("Using primer index:", classic_index, "(", classic_dataset.data_files[classic_index], ")")
        ...
        f_path = os.path.join(
            args.output_dir, f"rand_{classic_dataset.data_files[classic_index][len(classic_path)+5:]}.mid")
        decode_midi(rand_seq[0].cpu().numpy(), file_path=f_path)
        
    for pop_index in pop_index_list:
        primer, _, _ = pop_dataset[pop_index]
        primer = primer.to(get_device())
        print("Using primer index:", pop_index, "(", pop_dataset.data_files[pop_index], ")")
        ...
        f_path = os.path.join(
            args.output_dir, f"rand_{pop_dataset.data_files[pop_index][len(pop_path)+5:]}_{args.model_weights}.mid")
        decode_midi(rand_seq[0].cpu().numpy(), file_path=f_path)
